...
namespace eosio {
    ...
    namespace info_apis {
        permission_info::get_permission_results
        permission_info::get_permission(const get_permission_params &params) const {
            ...
            while (perm != permissions.end() && perm->owner == params.account_name) {
                ...
                while (link != permission_links.end()) {
                    ...
                    ilog("contract name: ${code}, action name: ${type}",
                         ("code", link->code)("type", link->message_type));
                ...
                ilog("actions: ${a}, result.permissions: ${p}", ("a", actions)("p", result.permissions));
                ++perm;
            }
            return result;
        }
        ...
        info_apis::get_block_info_results block_info::get_block_info(const block_info::get_block_info_params &) const {
            return {
                    db.head_block_time(),
                    db.fork_db_root().block_num(),//last_irreversible_block_num
                    get_ref_block_prefix(std::to_string(db.fork_db_root().block_num())),
            };
        }
